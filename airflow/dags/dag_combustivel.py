...
CLUSTER_NAME = 'stack-data-pipeline2'
REGION = 'us-central1'
PROJECT_ID = 'upheld-pursuit-371419'
CODE_BUCKET_NAME = 'data-pipeline-stack-combustiveis-brasil-pyspark-code2'
...
#operator responsável por conectar o airflow com o operator 
    get_data = SimpleHttpOperator(
        task_id='get_op',
        method='POST',
        http_conn_id='stack-data-pipeline',
        endpoint='download_combustivel',
        data=json.dumps({       #ENTENDER MELHOR ESTE RECURSO UTILIZADO - RENDERED TEMPLATE AIRFLOW - 
            "bucket_name": "data-pipeline-stack-combustiveis-brasil-raw2",
...
#definição das caracteristicas do bucket que irá ser criado
    CLUSTER_CONFIG = {
        "master_config": {
            "num_instances": 1,
    ...
    PYSPARK_JOB = {
        ...
        "pyspark_job": {
            ...
            "args": [
                '--path_input',
                "gs://data-pipeline-stack-combustiveis-brasil-raw2/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/ca-{{ dag_run.logical_date.strftime('%Y') }}-{{ '01' if dag_run.logical_date.month <= 6 else '02'}}.csv",
                '--path_output',
                "gs://data-pipeline-stack-combustiveis-brasil-curated2/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/",
                '--file_format', 'parquet',
